...
from wtforms import Form, FloatField, BooleanField, SelectField, StringField, RadioField, validators
...
class InputForm(Form):
    ...
    radio_field = RadioField(
        choices=[('radio1', 'firstRadio'), ('radio2', 'secondRadio')],
        validators=[validators.InputRequired()]
    )
    select_field = SelectField(
        choices=[('select1', 'select1'), ('select2', 'select2')],
        validators=[validators.InputRequired()]
    )
    boolean_field = BooleanField()
    text_field = StringField(validators=[validators.InputRequired()])

# Controller untuk route utama
